...
gp_and_practice_codes_df = pd.concat([gp_codes_df, practice_codes_df], ignore_index=True)

# Truncate strings to 50 and 35 characters
gp_and_practice_codes_df.iloc[:, 1] = gp_and_practice_codes_df.iloc[:, 2].apply(lambda x: x[:50] if len(x) > 50 else x)
gp_and_practice_codes_df.iloc[:, 2] = gp_and_practice_codes_df.iloc[:, 3].apply(lambda x: x[:35] if len(x) > 35 else x)

gp_and_practice_codes_filepath = Path("tables") / "ukrdc_ods_gp_codes" / "gp_and_prac_ods.csv"
gp_and_practice_codes_df.to_csv(gp_and_practice_codes_filepath, index=False, header=False)
